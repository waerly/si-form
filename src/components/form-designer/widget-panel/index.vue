              ...
              <draggable tag="ul" :list="containers" :group="{name: 'dragGroup', pull: 'clone', put: false}"
                ...
                <li v-for="(ctn, index) in containers" :key="index" class="container-widget-item"
                  ...
                  <span><svg-icon :icon-class="ctn.icon"
                                  class-name="color-svg-icon"/>{{ i18n2t(`designer.widgetLabel.${ctn.type}`, `extension.widgetLabel.${ctn.type}`) }}</span>
                </li>
              </draggable>
            </el-collapse-item>
              ...
              <draggable tag="ul" :list="basicFields" :group="{name: 'dragGroup', pull: 'clone', put: false}"
                ...
                <li v-for="(fld, index) in basicFields" :key="index" class="field-widget-item" :title="fld.displayName"
                  ...
                  <span><svg-icon :icon-class="fld.icon"
                                  class-name="color-svg-icon"/>{{ i18n2t(`designer.widgetLabel.${fld.type}`, `extension.widgetLabel.${fld.type}`) }}</span>
                </li>
              </draggable>
            </el-collapse-item>
...
export default {
  ...
  methods: {
    ...
    //根据类型新增
    addFieldByWidgetType(widgetType) {
      const widget = [...this.basicFields,this.advancedFields,this.containers,this.customFields].find(u => u.type === widgetType && u);
      if (!widget) {
        this.$message.error("请核实当前类型是否正确！")
      }
      this.designer.addFieldByDbClick(widget);
    },
