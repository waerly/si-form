                ...
                <el-tabs v-model="activeName" @tab-click="handleTabClick">
                  <el-tab-pane label="组织机构" name="fraOrg">
                    ...
                    <el-tree
                      ...
                      @check="handleSelectionChangeForDept"
                  ...
                  <el-tab-pane label="当前用户部门" name="user">
                    <el-input
                      clearable
                      placeholder="输入关键字进行过滤"
                      v-model="filterText">
                    ...
                    <el-table
                      ...
                      @selection-change="handleSelectionChangeForCurrentDept"
                      @current-change="handleCurrentChangeForCurrentDept"
                      @row-click="handleCurrentChangeForCurrentDept"
                    >
                      <el-table-column
          ...
          <el-main>
            <div v-show="activeName==='fraOrg'">
              <el-table
                ...
                @current-change="handleCurrentChangeForDept"
              >
                <el-table-column
                  :type="getTypeForColumn()"
                  width="55">
...
export default {
  ...
  data() {
    return {
      ...
      multipleSelection: [],
      multipleSelectionMap: {},//id,userName,dept
  ...
  methods: {
    ...
    initData() {
      this.loadCurrentDept();
      this.loadDept();//() => this.initDefaultChecked()
      this.$nextTick(() => {
    ...
    initDefaultChecked(data) {
      ...
      if (!isListEmpty(this.defaultChecked)) {
        ...
        this.defaultChecked.forEach(id => {
          ...
          if (self.deptTreeDataMap.hasOwnProperty(id)) {
            self.multipleSelectionMap[id] = self.deptTreeDataMap[id];
            //选中用户
            if (self.isMulti()) {
              self.deptUserSelectionStateToUserState(id, true);
              self.currentDeptUserSelectionStateToUserState(id, true);
            } else {
              //单选
              self.setCurrentForDept(id);
              self.setCurrentForCurrentDept(id);
            }
          }
        });
    ...
    //通过部门树查找部门
    getDeptTreeDataForDeptId(deptId) {
      // eslint-disable-next-line no-prototype-builtins
      if (this.deptTreeDataMap.hasOwnProperty(deptId)) {
        return this.deptTreeDataMap[deptId];
      }
      return null;
    },
    //通过部门id查找部门列表中对象
    getCurrentDeptDataForId(id) {
      // eslint-disable-next-line no-prototype-builtins
      if (this.tableDataMap.hasOwnProperty(id)) {
        return this.tableDataMap[id];
      }
      return null;
    },
    //通过用户查找用户Id列表中对象
    ...
    //通过架构中用户选中用户列表中用户-多选
    currentDeptUserSelectionStateToUserState(id, checkState = true) {
      //fra转用户列表中数据
      let row = this.getCurrentDeptDataForId(id);
      if (row) {
        this.$refs[this.tableRefs.onlyUserTable]&&this.$refs[this.tableRefs.onlyUserTable].toggleRowSelection(row, checkState);
      }
    },
    //多选
    deptUserSelectionStateToUserState(id, checkState = true, deep) {
      //fra转用户列表中数据
      // let row = this.getDeptTreeDataForDeptId(id);
      if (id) {
        this.$refs[this.tableRefs.fraOrgTreeNode].setChecked(id, checkState, deep);
      }
    },
    deptUserSelectionStateToUserStateForIds(ids, checkState = true) {
      //fra转用户列表中数据
      // let row = this.getDeptTreeDataForDeptId(id);
      if (ids) {
        this.$refs[this.tableRefs.fraOrgTreeNode].setCheckedKeys(ids, checkState);
      }
    },
    //通过架构中用户选中用户列表中用户
    ...
    //多选-选中部门树时
    handleSelectionChangeForDept(checkedObj, p) {
      let checked = p.checkedNodes;
      if (this.type === "radio") {
        return;
      }
      let fraCheckedMap = {};
      checked.forEach(c => {
        fraCheckedMap[c.id] = c;
        this.deptUserSelectionStateToUserState(c.id, true)
        this.currentDeptUserSelectionStateToUserState(c.id, true)
        // eslint-disable-next-line no-prototype-builtins
        if (!this.multipleSelectionMap.hasOwnProperty(c.id)) {
          this.multipleSelectionMap[c.id] = c;
        }
      })
      //去除部门中数据的选中
      for (let key in this.deptTreeDataMap) {
        let d = this.deptTreeDataMap[key];
        // eslint-disable-next-line no-prototype-builtins
        if (!fraCheckedMap.hasOwnProperty(d.id)) {
          this.deptUserSelectionStateToUserState(d.id, false)
          this.currentDeptUserSelectionStateToUserState(d.id, false)
          delete this.multipleSelectionMap[d.id];
        }
      }
      this.selectionMapToArray();
    },
    //多选-选择当前部门时
    handleSelectionChangeForCurrentDept(checked) {
      if (this.type === "radio") {
        return;
      }
      let fraCheckedMap = {};
      ...
      this.tableData.forEach(user => {
        // eslint-disable-next-line no-prototype-builtins
        if (!fraCheckedMap.hasOwnProperty(user.id)) {
          //去除部门中数据的选中
          this.deptUserSelectionStateToUserState(user.id, false)
          this.currentDeptUserSelectionStateToUserState(user.id, false)
          delete this.multipleSelectionMap[user.id];
        }
      })
      this.selectionMapToArray();
    },
    ...
    //设置当前部门-单选时 todo 当前部门
    setCurrentForDept(deptId) {
      let row = this.getDeptTreeDataForDeptId(deptId);
      if (row) {
        this.$refs[this.tableRefs.fraOrgTreeNode].setCurrentNode(row);
      }
    },
    setCurrentForCurrentDept(userId) {
      let row = this.getUserDataForUserId(userId);
      if (row) {
        this.$refs[this.tableRefs.onlyUserTable] && this.$refs[this.tableRefs.onlyUserTable].setCurrentRow(row);
      }
    },
    ...
    handleCurrentChangeForCurrentDept(val) {
      if (this.type === "multi") {
        return;
      }
      this.currentRow = val;
      ...
      checked.forEach(c => {
        fraCheckedMap[c.id] = c;
        this.setCurrentForCurrentDept(c.id);
        // eslint-disable-next-line no-prototype-builtins
        if (!this.multipleSelectionMap.hasOwnProperty(c.id)) {
          this.multipleSelectionMap[c.id] = c;
        }
    ...
    //选中部门单选
    handleCurrentChangeForDept(val) {
      if (this.type === "multi") {
        return;
      }
      this.currentRow = val;
      ...
      //去除选中的数据
      for (let k in this.deptTreeDataMap) {
        let d = this.deptTreeDataMap[k];
        // eslint-disable-next-line no-prototype-builtins
        if (!fraCheckedMap.hasOwnProperty(d.id)) {
          delete this.multipleSelectionMap[d.id];
        }
    ...
    handlerTreeNodeClick(node) {
      if (node) {
        this.handleCurrentChangeForDept(node);
    ...
    iterateDept(data) {
      data.forEach(d => {
        if (d.children && d.children.length > 0) {
          this.deptTreeDataMap[d.id] = d;
          this.iterateDept(d.children);
        } else {
          this.deptTreeDataMap[d.id] = d;
        }
      })
    ...
    //加载部门
    loadDept(callback) {
      axios.get(`${this.getCtx()}base/general/dept`).then(res => {
        if (res.status === 200 && res.data) {
          let resData = res.data;
          if (resData.code === 0) {
            let data = resData.data;
            this.fraOrgData = data;
            this.iterateDept(this.fraOrgData);
    ...
    loadCurrentDept() {
      let self = this;
      axios.get(`${this.getCtx()}base/general/current_dept`).then(res => {
        if (res.status === 200 && res.data) {
          let resData = res.data;
          if (resData.code === 0) {
            let data = resData.data;
            ...
            data.forEach(d => {
              let dept = {id: d.deptId, label: d.deptName};
              self.tableDataMap[dept.id] = dept;
              self.tableData.push(dept);
              self.filterTableData.push(dept);
              setTimeout(() => {
                // eslint-disable-next-line no-prototype-builtins
                if (self.multipleSelectionMap.hasOwnProperty(dept.id)) {
                  if (this.isMulti()) {
                    self.currentDeptUserSelectionStateToUserState.call(self, dept.id, true);
                  } else {
                    self.setCurrentForCurrentDept(dept.id);
                  }
                }
              }, 400);
